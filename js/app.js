class ZOMATO{
  constructor(){
    this.api='1727458833450f9278ea1554841f02b2';
    this.header={
      method:'GET',
      headers:{
        "user-key":this.api,
...
class UI{
  ...
  getRestaurants(restaurants){
    ...
    if(restaurants.length === 0){
      this.showFeedback('no such category in selected city');
    }
    else{
      console.log(restaurants[0].restaurant);
      this.restaurantList.innerHTML = '';
      restaurants.forEach((restaurant) => {
        const {thumb:img,name,location:{address},user_rating:{aggregate_rating, rating_text},cuisines,average_cost_for_two:cost,menu_url,url} = restaurant.restaurant;
       ...
       <div class="row p-3">
        ...
        <div class="col-1">
         <div class="badge badge-success">
          ${aggregate_rating}<br>${rating_text}
         </div>
        </div>
       </div>
       <hr>
